...
export class InterstellarWarpScene{
 ...
 update(dt,features,musicalState,progress=0){
  if(!this.cinematicEncounters&&this.scene.userData.uruxCinematicEncounterSystem)this.cinematicEncounters=this.scene.userData.uruxCinematicEncounterSystem;
  const direction=this.musicDirector.update(features,musicalState,progress,dt),base=this.director.update(features,musicalState,dt);this.narrative=this.journey.update(dt,features,musicalState,progress);this.state=this.journey.apply(base,this.narrative);
  this.scene.userData.uruxMusicDirection=direction;if(this.cinematicEncounters&&!this.cinematicEncounters.externalDriver)this.cinematicEncounters.update(dt);
  ...
  this.musicDirection={...direction,encounterRelief,nebulaAppliedScale:this.state.nebulaDirectorScale};this.scene.userData.uruxMusicDirection=this.musicDirection;
 ...
 dispose(){removeEventListener('resize',this.resize);if(this.cinematicEncounters&&!this.cinematicEncounters.externalDriver)this.cinematicEncounters.dispose?.();if(globalThis.__URUX_SCENE_INSTANCE__===this)globalThis.__URUX_SCENE_INSTANCE__=null;this.warpField.dispose();this.nebulaPass.dispose();this.renderer.dispose();this.renderer.domElement.remove();}
 get performanceStats(){return{pixelRatio:this.renderer.getPixelRatio(),drawCalls:this.renderer.info.render.calls,triangles:this.renderer.info.render.triangles,musicDirection:this.musicDirection,cinematic:this.cinematicEncounters?.stats||this.scene.userData.uruxCinematicEncounterSystem?.stats||null,warp:this.warpField.stats};}
}