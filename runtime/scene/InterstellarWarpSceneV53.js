...
export class InterstellarWarpScene{
 ...
 update(dt,features,musicalState,progress=0){
  ...
  this.scene.userData.uruxMusicDirection=direction;if(!this.cinematicEncounters.externalDriver)this.cinematicEncounters.update(dt);
 ...
 dispose(){removeEventListener('resize',this.resize);this.cinematicEncounters?.dispose?.();this.warpField.dispose();this.nebulaPass.dispose();this.renderer.dispose();this.renderer.domElement.remove();}
 get performanceStats(){return{pixelRatio:this.renderer.getPixelRatio(),drawCalls:this.renderer.info.render.calls,triangles:this.renderer.info.render.triangles,musicDirection:this.musicDirection,cinematic:this.cinematicEncounters?.stats||null,warp:this.warpField.stats};}
}