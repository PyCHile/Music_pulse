...
export class ThreeNebulaParticleFX{
  constructor(scene){this.scene=scene;this.system=null;this.api=null;this.emitters=new Map();this.ready=false;this.failed=false;this.stats={ready:false,emitters:0,count:0,engine:'three-nebula',lastEffect:null};}
  async init(){if(this.ready||this.failed)return this.ready;try{const mod=await import('three-nebula');this.api=mod;const System=mod.default||mod.System;this.system=new System();this.system.addRenderer(new mod.SpriteRenderer(this.scene,THREE));this.ready=true;this.stats.ready=true;runtimeCapabilities.mark('three-nebula',true,{version:'12.1.0',renderer:'SpriteRenderer'});return true;}catch(error){this.failed=true;runtimeCapabilities.mark('three-nebula',false,{version:'12.1.0',error:String(error)});console.warn('[URUX] three-nebula unavailable',error);return false;}}
