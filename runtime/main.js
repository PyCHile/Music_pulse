...
let started=false,starting=false,runtimeReady=false,runtimeInitPromise=null,renderLoopStarted=false,last=performance.now(),lastRendered=0,hideTimer=0,features={...DEFAULT_FEATURES},musicalState={phase:'CALM',intensity:0,trend:0,duration:0},journeySnapshot=null,lastNebulaDirective=null;
...
const minFrameMs=navigator.webdriver?50:(mobile?1000/30:0);

const nextPaint=()=>new Promise(resolve=>requestAnimationFrame(()=>setTimeout(resolve,0)));
const boot=()=>window.__URUX_BOOT__||{};
function baseDiagnostics(){const b=boot();return{runtimeInitializing:!runtimeReady,startState:starting?'initializing':(started?'audio-ready':'waiting-for-enter'),backendConfigured:Boolean(journeyAgent?.diagnostics?.backendConfigured),backendStatus:journeyAgent?.diagnostics?.backendStatus??'—',llmActive:false,runtimeVersion:b.version||'—',runtimeBuild:b.build||'—',runtimeReleasedAt:b.releasedAt||null,cacheCleared:Boolean(b.cacheCleared),serviceWorkersCleared:Boolean(b.serviceWorkersCleared),bootStartedAt:b.startedAt||null};}
function fullDiagnostics(){if(!runtimeReady||!journeyAgent||!scene||!cinematicEncounters||!nebulaBridge||!runtimeCapabilities)return baseDiagnostics();const b=boot();return{...journeyAgent.diagnostics,runtimeInitializing:false,llmActive:Boolean(journeyAgent.diagnostics.backendConfigured&&journeyAgent.diagnostics.acceptedBatches>0),cinematic:cinematicEncounters.stats,nebulaDirective:nebulaBridge.stats,nebulaRaySteps:scene.warpField?.nebula?.raySteps||0,nebulaLightSteps:scene.warpField?.nebula?.lightSteps||0,scientificAstronomy:scientificAstronomy?.snapshot?.()||null,capabilities:runtimeCapabilities.snapshot(),allowedAgentCapabilities:runtimeCapabilities.allowedForAgent(),canonicalLegacyStage:scene.state?.journeyStage||null,runtimeVersion:b.version||'—',runtimeBuild:b.build||'—',runtimeReleasedAt:b.releasedAt||null,cacheCleared:Boolean(b.cacheCleared),serviceWorkersCleared:Boolean(b.serviceWorkersCleared),bootStartedAt:b.startedAt||null};}
...
window.URUXJourney={
 ...
 async backendHealth(){if(journeyAgent?.backend)return journeyAgent.backend.health();return{ok:false,status:'runtime_initializing'};},
...
async function initializeSpaceKit(SpaceKitAstronomicalLayer){if(spacekit)return spacekit;await nextPaint();try{spacekit=new SpaceKitAstronomicalLayer(spacekitHost);return spacekit;}catch(error){console.warn('URUX SpaceKit deferred initialization failed',error);return null;}}

async function initializeRuntime(){
 if(runtimeInitPromise)return runtimeInitPromise;
 runtimeInitPromise=(async()=>{
  status.textContent='Preparando motor visual…';
  await nextPaint();
  const [sceneModule,spacekitModule,agentModule,encounterModule,nebulaModule,scienceModule,capabilityModule]=await Promise.all([
   import('./scene/InterstellarWarpScene.js?v=20260813-44'),
   import('./spacekit/SpaceKitAstronomicalLayer.js?v=20260813-44'),
   import('./journey/SecureURUXJourneyAgent.js?v=20260813-10'),
   import('./journey/CinematicEncounterSystemV42.js?v=20260813-42'),
  ...
  status.textContent='Inicializando viaje…';
  await nextPaint();
  ...
  setTimeout(()=>void initializeSpaceKit(spacekitModule.SpaceKitAstronomicalLayer),1200);
  return true;
 })().catch(error=>{runtimeInitPromise=null;runtimeReady=false;boot().runtimeError=String(error);throw error;});
 return runtimeInitPromise;
}

enterBtn.addEventListener('click',async()=>{
 if(starting||started)return;
 starting=true;enterBtn.disabled=true;enterBtn.textContent='Iniciando…';status.textContent='Activando audio…';boot().enterAccepted=true;
 try{
  await audio.play();
  started=true;boot().audioUnlocked=true;
  await nextPaint();
  await initializeRuntime();
  startGate.classList.add('hidden');controls.hidden=false;showControls();
 }catch(err){console.error('[URUX] startup failed',err);started=false;starting=false;enterBtn.disabled=false;enterBtn.textContent='Tocar para iniciar';status.textContent='No fue posible iniciar. Toca nuevamente.';boot().runtimeError=String(err);}
});
...
function loop(now){
 ...
 if(!runtimeReady||!scene)return;
 if(minFrameMs&&now-lastRendered<minFrameMs)return;
 const dt=Math.min(.05,Math.max(.001,(now-last)/1000));last=now;lastRendered=now;
 if(started&&!audio.paused&&!audio.ended){features=audioReactive.sample(dt);musicalState=musical.update(features,dt);}else if(!started){features={...DEFAULT_FEATURES,energy:.06,bass:.04,high:.04,calmness:.94};musicalState={phase:'CALM',intensity:.06,trend:0,duration:0};}else if(audio.ended){features={...DEFAULT_FEATURES,energy:.025,bass:.012,high:.012,calmness:.98};musicalState={phase:'RECOVERING',intensity:.025,trend:-.02,duration:0};}
