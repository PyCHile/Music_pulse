...
export class WarpFieldManager{
  constructor(maxStars){
    ...
    this.group.add(this.starTunnel.group);
    const dustCount=Math.max(450,Math.floor(maxStars*.14));
    this.dustPositions=new Float32Array(dustCount*3);
    this.dustSpeeds=new Float32Array(dustCount);
    this.dustCount=dustCount;
    for(let i=0;i<dustCount;i++)this.resetDust(i,true);
    ...
    for(let i=0;i<dustCount;i++)sizes[i]=.8+Math.random()*1.6;
    this.dustGeometry.setAttribute('aSize',new THREE.BufferAttribute(sizes,1));
    this.dustMaterial=new THREE.ShaderMaterial({vertexShader:dustVertex,fragmentShader:dustFragment,transparent:true,blending:THREE.AdditiveBlending,depthWrite:false,uniforms:{uOpacity:{value:.12},uWarmth:{value:.12}}});
    this.dust=new THREE.Points(this.dustGeometry,this.dustMaterial);
    this.dust.frustumCulled=false;
    this.group.add(this.dust);
  }
  resetDust(i,initial=false){
    const theta=Math.random()*Math.PI*2,r=4+Math.random()*35,p=i*3;
    this.dustPositions[p]=Math.cos(theta)*r;
    this.dustPositions[p+1]=Math.sin(theta)*r;
    this.dustPositions[p+2]=initial?-10-Math.random()*130:-140-Math.random()*20;
    this.dustSpeeds[i]=.55+Math.random()*.55;
  }
  update(dt,state,features,vanishingPoint){
    this.nebula.update(dt,state,features,vanishingPoint);
    this.starTunnel.update(dt,state,features,vanishingPoint);
    this.dustMaterial.uniforms.uOpacity.value=.025+state.dustDensity*.12;
    this.dustMaterial.uniforms.uWarmth.value=Math.min(1,(features.lowMid||0)*.55+(features.mid||0)*.18);
    const speed=state.speed*.66;
    for(let i=0;i<this.dustCount;i++){
      const p=i*3;
      this.dustPositions[p+2]+=speed*this.dustSpeeds[i]*dt;
      if(this.dustPositions[p+2]>-1.5)this.resetDust(i,false);
    }
    this.dustGeometry.attributes.position.needsUpdate=true;
  }
  dispose(){
    this.nebula.dispose();
    this.starTunnel.dispose();
