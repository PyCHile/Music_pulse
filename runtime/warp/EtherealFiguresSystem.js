...
const REFERENCES=[
  {url:'./assets/ethereal/angel-01.png?v=20260812-png4',width:19.5,center:.28,maxOpacity:.30,x:-1.0,y:.25,z:-30},
  {url:'./assets/ethereal/angel-02.png?v=20260812-png4',width:24.0,center:.70,maxOpacity:.34,x:.9,y:.15,z:-33}
];
export class EtherealFiguresSystem{
  constructor(){this.group=new THREE.Group();this.layers=[];this.time=0;this.readyCount=0;this.loader=new THREE.TextureLoader();for(const ref of REFERENCES){const material=new THREE.SpriteMaterial({color:0xffffff,transparent:true,opacity:0,depthWrite:false,depthTest:false,alphaTest:.035,blending:THREE.AdditiveBlending,toneMapped:true});const sprite=new THREE.Sprite(material);sprite.position.set(ref.x,ref.y,ref.z);sprite.renderOrder=26;this.group.add(sprite);const layer={...ref,material,sprite,texture:null,aspect:1};this.layers.push(layer);this.loadReference(layer);}this.group.position.set(0,0,0);this.group.visible=false;}
