...
export class CloudNebulaVolume{
 constructor(){this.group=new THREE.Group();this.volumes=[];this.time=0;this.crossings=0;this.cameraCrossings=0;this.nearClouds=0;this.maxOpacity=0;this.peakOpacity=0;this.ready=true;this.mobile=/iPad|iPhone|iPod|Android/i.test(navigator.userAgent||'')||innerWidth<800;const volumeCount=this.mobile?4:5,particlesPerVolume=this.mobile?420:520;for(let i=0;i<volumeCount;i++)this.createVolume(i,particlesPerVolume,i%3===1);this.totalParticles=volumeCount*particlesPerVolume;}
 createVolume(i,count,emissive){const seed=71+i*47.3,geometry=buildVolumeGeometry(seed,count,this.mobile,emissive),material=makeMaterial(this.mobile,emissive),points=new THREE.Points(geometry,material);points.frustumCulled=false;points.renderOrder=-920+i;const z=i===0?-18:-18-i*(this.mobile?58:54),x=signed(seed+2.4)*(i===0?3.0:7.5),y=signed(seed+5.1)*(i===0?2.2:5.5);points.position.set(x,y,z);points.rotation.z=signed(seed+8.2)*.42;this.group.add(points);this.volumes.push({points,geometry,material,seed,emissive,speed:.80+hash(seed+13.2)*.26,spin:signed(seed+19.4)*.00022,lastZ:z});}
 recycle(q){let far=-175;for(const v of this.volumes)far=Math.min(far,v.points.position.z);q.points.position.z=far-(this.mobile?48:44)-hash(q.seed+this.crossings*1.17)*16;q.points.position.x=signed(q.seed+this.crossings*2.31)*8;q.points.position.y=signed(q.seed+this.crossings*3.17)*6;q.points.rotation.z=signed(q.seed+this.crossings*4.11)*.48;q.lastZ=q.points.position.z;this.crossings++;}
 update(dt,state){this.time+=dt;const n=clamp01(state.nebulaPresence||0),g=clamp01(state.galaxyReveal||0),light=clamp01(state.livingLight||0),tunnel=clamp01(state.tunnelDrive||0),fade=1-clamp01(state.finalFade||0),presence=Math.min(1,.93+n*.18+g*.12+light*.10+tunnel*.08)*fade,speed=Math.max(4.1,(state.speed||8)*.92);this.nearClouds=0;this.maxOpacity=0;for(const q of this.volumes){q.points.position.z+=speed*q.speed*dt;let z=q.points.position.z;if(q.lastZ<0&&z>=0)this.cameraCrossings++;q.lastZ=z;if(z>38){this.recycle(q);z=q.points.position.z;}const farFade=smooth01((z+245)/92),nearFade=1-smooth01((z-22)/38),nearBoost=.96+1.16*Math.exp(-Math.pow((z+16)/29,2)),base=q.emissive?.50:.67,cap=q.emissive?.74:.94,opacity=Math.min(cap,base*presence*farFade*nearFade*nearBoost);q.material.uniforms.uOpacity.value=opacity;q.material.uniforms.uTime.value=this.time;q.points.rotation.z+=q.spin*dt*60;q.points.position.x+=Math.sin(this.time*.021+q.seed)*dt*.020;q.points.position.y+=Math.cos(this.time*.017+q.seed*.71)*dt*.016;if(z>-62&&z<24&&opacity>.10)this.nearClouds++;this.maxOpacity=Math.max(this.maxOpacity,opacity);this.peakOpacity=Math.max(this.peakOpacity,opacity);}}
