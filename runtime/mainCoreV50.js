...
function scheduleSecondarySystems(){const launch=()=>void initializeSecondarySystems();if(!mobile){setTimeout(launch,80);return;}if('requestIdleCallback'in window)window.requestIdleCallback(launch,{timeout:9000});else setTimeout(launch,7000);}
...
async function initializeRuntime(){
 ...
 runtimeInitPromise=(async()=>{
  ...
  const sceneModule=mobile?await import('./scene/MobileInterstellarWarpScene.js?v=20260813-47'):await import('./scene/InterstellarWarpScene.js?v=20260813-46');
  ...
  startRenderLoop();scheduleSecondarySystems();return true;
...
enterBtn.addEventListener('click',async()=>{
 ...
 try{await audio.play();started=true;boot().audioUnlocked=true;startGate.classList.add('hidden');controls.hidden=false;setPhase('entered');await nextPaint();await initializeRuntime();starting=false;enterBtn.textContent='Entrar';showControls();}
...
function loop(now){
 requestAnimationFrame(loop);if(!runtimeReady||!scene||!frameDue(now))return;
 const gap=perfStats.lastRenderAt?now-perfStats.lastRenderAt:frameInterval||16.7;perfStats.lastRenderAt=now;perfStats.frames++;perfStats.maxGapMs=Math.max(perfStats.maxGapMs,gap);if(frameInterval&&gap>frameInterval*1.55)perfStats.lateFrames++;
 const dt=Math.min(.05,Math.max(.001,(now-last)/1000));last=now;const workStart=performance.now();
 if(started&&!audio.paused&&!audio.ended){if(!audioSampleInterval||now-lastAudioSampleAt>=audioSampleInterval){const sampleDt=lastAudioSampleAt?Math.min(.12,(now-lastAudioSampleAt)/1000):dt;lastAudioSampleAt=now;features=audioReactive.sample(sampleDt);musicalState=musical.update(features,sampleDt);}}
 else if(!started){features={...DEFAULT_FEATURES,energy:.06,bass:.04,high:.04,calmness:.94};musicalState={phase:'CALM',intensity:.06,trend:0,duration:0};}
 else if(audio.ended){features={...DEFAULT_FEATURES,energy:.025,bass:.012,high:.012,calmness:.98};musicalState={phase:'RECOVERING',intensity:.025,trend:-.02,duration:0};}
