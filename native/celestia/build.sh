...
for _ in range(40):
    try:
        ...
        for module in ('celengine','celephem','celrender','celscript'):
...
python "$ROOT/science/export_celestia_static_runtime.py" \
  --base "http://127.0.0.1:18080" --token "$SIDECAR_TOKEN" --output "$OUT" \
...
payload={
 ...
 "headlessUses":["photometry","physical constants","equatorial coordinate conversion","orbital anomaly solving","mean ecliptic obliquity","CelestiaContent catalog indexing","star/deep-sky/solar-system/model/texture/script resource discovery"],
 ...
 "note":"Literal libcelestia is compiled and executed in GitHub Actions. Native calculations and official CelestiaContent inventory are exported to static assets consumed from GitHub Pages; no permanent server or paid container is required."
}
